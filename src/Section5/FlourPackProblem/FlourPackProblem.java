package Section5.FlourPackProblem;
...
//  For example, if goal = 9, bigCount = 2, and smallCount = 0, the method should return false because each bigCountbag
//  is 5 kilos and cannot be divided. However, if goal = 9, bigCount = 1, and smallCount = 5,
...
//canPack (-3, 2, 12); should return false since bigCount is negative.
    ...
    public static boolean canPack(int bigCount, int smallCount, int goal) {
        ...
        int total = 0;
        int remainingBigBags = bigCount;
        while ((total + 5 <= goal) && (remainingBigBags > 0)) {
            total += 5;
            remainingBigBags--;
        }

        int remainingSmallBags = smallCount;
        while ((total < goal) && (remainingSmallBags > 0)) {
            total++;
            remainingSmallBags--;
        }
